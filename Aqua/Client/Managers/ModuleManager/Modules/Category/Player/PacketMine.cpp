...
void PacketMine::onD2DRender() {
    ...
    Vec2<float> screenPos = ImGuiUtils::WorldToScreen(center, screenSize);
    
    if (screenPos.x < 0 || screenPos.y < 0)
        return;
        
    float textSize = 1.f;
    float textWidth = ImGuiUtils::getTextWidth(text, textSize);
    Vec2<float> textPos(screenPos.x - textWidth / 2.f, screenPos.y);
    ImGuiUtils::drawText(textPos, text, UIColor(255, 255, 255), textSize);
}