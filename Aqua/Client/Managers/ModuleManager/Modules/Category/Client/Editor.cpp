...
Editor::Editor() : Module("Editor", "Edit UI component positions and sizes", Category::CLIENT, 88) {
    defaultWidgetConfigs["potion_effects"] = {Vec2<float>(100, 200), 1.0f};
    defaultWidgetConfigs["keystrokes"] = {Vec2<float>(200, 200), 1.0f};
    defaultWidgetConfigs["hud_info"] = {Vec2<float>(10, 10), 1.0f};
}
...
void Editor::onImGuiRender(ImDrawList* drawlist) {
...
void Editor::renderWidgetMarkers() {
    for(const auto& widget : widgets) {
        ...
        ImGuiUtils::renderCircleFilled(widget.position, markerColor.toImColor(), 8.0f);
    }
}
...
void Editor::renderSizeSliders() {
    for (const auto& widget : widgets) {
        ...
        ImGuiUtils::renderCircleFilled(sliderPos, sliderColor.toImColor(), 5.0f);
    }
}
...
void Editor::renderWidgetInfo() {
    if (selectedWidget) {
        ...
        float textWidth = ImGuiUtils::getTextWidth(info, 0.9f);
        ...
        ImGuiUtils::fillRectangle(bgRect, UIColor(0, 0, 0, 150), 3.0f);
        
        ImGuiUtils::drawText(realInfoPos, info, UIColor(255, 255, 255, 255), 0.9f);
        
        Vec2<float> hintPos = Vec2<float>(10, 35);
        std::string hint = "Left Click: Drag | Right Click: Cancel | Drag Green Circle: Resize";
        ImGuiUtils::drawText(hintPos, hint, UIColor(200, 200, 200, 180), 0.7f);
    }
}
