...
void ImGuiUtils::drawText(Vec2<float> textPos, std::string textStr, UIColor color, float textSize, bool glow, float thick_ness) {
...
void ImGuiUtils::drawRectangleShadow(const Vec4<float>& rectPos, UIColor color, float shadow_thickness, const ImVec2& shadow_offset, float rounding, ImDrawFlags flags) {
    //drawlist->AddShadowRect(ImVec2(rectPos.x, rectPos.y), ImVec2(rectPos.z, rectPos.w), color.toImColor(), shadow_thickness, shadow_offset, flags, rounding);
}
...
void ImGuiUtils::drawBox(const AABB& blockAABB, UIColor color, UIColor lineColor, float lineWidth, bool fill, bool outline) {
    ...
    Vec2<float> fov = GI::getClientInstance()->getFov();
	...
	if (fill) {
		...
		for (int i = 0; i < 8; i++) {
			Vec2<float> screen;
            if(Matrix::WorldToScreen(origin, screen)) {
...
bool ImGuiUtils::worldToScreen(const Vec3<float>& pos, Vec2<float>& out) {
    return Matrix::WorldToScreen(pos, out);
}

void ImGuiUtils::renderCircle(Vec2<float> pos, ImColor color, float radius, float thickness, int num_seg) {
                drawlist->AddCircle(pos.toImVec2(), radius, color, num_seg, thickness);
}

void ImGuiUtils::renderCircleFilled(Vec2<float> pos, ImColor color, float radius, int num_seg) {
                drawlist->AddCircleFilled(pos.toImVec2(), radius, color, num_seg);
}

void ImGuiUtils::renderCircleShadow(Vec2<float> pos, ImColor color, float radius, float thickness, bool fill, int num_seg, ImVec2 offset) {
}

Vec2<float> ImGuiUtils::WorldToScreen(const Vec3<float>& worldPos, const Vec2<float>& screenSize) {
